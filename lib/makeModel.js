...
const { ObjectId, Decimal128, Map, Mixed } = require('mongoose').Types
...
exports.default = class ${opts.name} extends Model {
    ...
    static getModelParamKey () {
        return 'id'
    }

    static getRouteParamKey () {
        return 'identifier'
    }
...
if (!config('database')) {
    console.log('Database Config doesn\'t exist. Creating config with factory settings...')
    let configData = 
`
module.exports = {
    default: 'connection1',
    ...
    connections: {
        ...
        connection1: {
            connString: env('DB_STRING'),
            dbName: env('DB_NAME'), // ignore when connection-string already has database name
        }

    }
}`
    fs.writeFileSync(app().configPath('database.js'), configData)
}
