...
`   
    ...
    async all ({ req, res, next }) {
        ...
        res.status(200).json({ status: "success"', data: ${pluralWord}.lean() })
    }
     ...
     */
    async post (ctx) {
        let binding = ModelBinding.withForm(ctx, ${opts.name})
        let validationErrors = binding.document.validateSync()
    ...
    async delete (ctx) {
        let binding = await ModelBinding.withRoute(ctx, ${opts.name})
        await ${singularWord}.deleteDocument()
    }
`) + 
`
}
